...
- 알림 신청 대상은 `SCHEDULED` 상태의 타임딜입니다. 판매 중(`ACTIVE`)인 타임딜에는 오픈 알림 버튼을 노출하지 않습니다.
- 타임딜 시작 시각까지 15분 이하로 남은 경우에는 알림 발송 준비 시간이 부족하므로 구독할 수 없습니다.
- 15분 이하 타임딜은 버튼을 숨기지 않고 비활성화 상태로 표시하며, 사용자에게 `곧 오픈되어 알림 신청 불가`를 안내합니다.
- 15분 초과 타임딜만 구독 상태를 조회하고 오픈 알림 신청·해제를 허용합니다.
- 알림 신청 여부는 타임딜별 단건 구독 상태 API를 기준으로 표시합니다. 전체 구독 목록을 프론트에서 추정하거나 로컬 상태만으로 확정하지 않습니다.
