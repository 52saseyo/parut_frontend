...
타임딜의 공개 조회는 `/api/v1/time-deals`를 사용하고, 판매자 전용 생성·목록·수정·삭제·중지·재고 작업은 `/api/v1/seller/timed-deals` 하위 endpoint를 사용합니다. 판매자 API의 목록은 cursor 기반이며 `cursor`, `cursorId`, `size`를 사용합니다.
